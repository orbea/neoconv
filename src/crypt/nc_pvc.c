 ...
 * Ported to ISO C99 from MAME (prot_pvc.cpp) for libneoconv.
...
void neoconv_mslug5_decrypt_68k(uint8_t* rom, uint32_t size)
{
	...
	for (int i = 0x100000; i < 0x0800000; i += 4)
	{
		...
		rom16 = nc_bitswap(rom16, 16, 15, 14, 13, 12, 10, 11, 8, 9, 6, 7, 4, 5, 3, 2, 1, 0);
	...
	{
		int ofst = (i & 0xf0) + nc_bitswap((i & 0x0f), 8, 7, 6, 5, 4, 1, 0, 3, 2);
		memcpy(&rom[i * 0x10000], &buf[ofst * 0x10000], 0x10000);
	}
	for (int i = 0x100000; i < 0x800000; i += 0x100)
	{
		int ofst = (i & 0xf000ff) + ((i & 0x000f00) ^ 0x00700) + (nc_bitswap(((i & 0x0ff000) >> 12), 8, 5, 4, 7, 6, 1, 0, 3, 2) << 12);
...
void neoconv_svc_px_decrypt(uint8_t* rom, uint32_t size)
{
	...
	for (int i = 0; i < 0x0100000 / 0x10000; i++)
	{
		int ofst = (i & 0xf0) + nc_bitswap((i & 0x0f), 8, 7, 6, 5, 4, 2, 3, 0, 1);
		memcpy(&rom[i * 0x10000], &buf[ofst * 0x10000], 0x10000);
	}

	for (int i = 0x100000; i < 0x800000; i += 0x100)
	{
		int ofst = (i & 0xf000ff) + ((i & 0x000f00) ^ 0x00a00) + (nc_bitswap(((i & 0x0ff000) >> 12), 8, 4, 5, 6, 7, 1, 0, 3, 2) << 12);
		memcpy(&rom[i], &buf[ofst], 0x100);
	}
	memcpy(buf, rom, rom_size );
	memcpy(&rom[0x100000], &buf[0x700000], 0x100000);
...
void neoconv_kof2003_decrypt_68k(uint8_t* rom, uint32_t size)
{
	...
	{
		uint16_t rom16 = rom[BYTE_XOR_LE(i+1)] | rom[BYTE_XOR_LE(i+2)] << 8;
		rom16 = nc_bitswap(rom16, 16, 15, 14, 13, 12, 5, 4, 7, 6, 9, 8, 11, 10, 3, 2, 1, 0);
		rom[BYTE_XOR_LE(i+1)] = rom16 & 0xff;
		rom[BYTE_XOR_LE(i+2)] = rom16 >> 8;
	}
	...
	for (int i = 0; i < 0x0100000 / 0x10000; i++)
	{
		int ofst = (i & 0xf0) + nc_bitswap((i & 0x0f), 8, 7, 6, 5, 4, 0, 1, 2, 3);
		memcpy(&buf[i * 0x10000], &rom[ofst * 0x10000], 0x10000);
	}

	for (int i = 0x100000; i < 0x900000; i += 0x100)
	{
		int ofst = (i & 0xf000ff) + ((i & 0x000f00) ^ 0x00800) + (nc_bitswap(((i & 0x0ff000) >> 12), 8, 4, 5, 6, 7, 1, 0, 3, 2) << 12);
...
void neoconv_kof2003h_decrypt_68k(uint8_t* rom, uint32_t size)
{
	...
	for (int i = 0x100000; i < 0x900000; i += 0x100)
	{
		int ofst = (i & 0xf000ff) + ((i & 0x000f00) ^ 0x00400) + (nc_bitswap(((i & 0x0ff000) >> 12), 8, 6, 7, 4, 5, 0, 1, 2, 3) << 12);
		memcpy(&buf[i], &rom[ofst], 0x100);
	}
	memcpy(&rom[0x000000], &buf[0x000000], 0x100000);
	memcpy(&rom[0x100000], &buf[0x800000], 0x100000);

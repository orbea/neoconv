...
static void gfx_decrypt(uint8_t* rom, uint32_t rom_size, int extra_xor)
{
	unsigned rpos;
...
static int m1_address_scramble(int address, uint16_t key)
{
	...
	aux ^= nc_bitswap(key, 16,12,0,2,4,8,15,7,13,10,1,3,6,11,9,14,5);
	aux = nc_bitswap(aux, 16,
	...
	aux = nc_bitswap(aux, 16, 7,15,14,6,5,13,12,4,11,3,10,2,9,1,8,0);
...
void neoconv_cmc50_m1_decrypt(uint8_t* romcrypt, uint32_t romcrypt_size, uint8_t* romaudio, uint32_t romaudio_size)
{
	...
	uint8_t *buffer = (uint8_t *)calloc((size_t)(rom_size), sizeof(uint8_t)); 
	...
	uint16_t key = nc_sum16(rom, 0x10000);
